...
- **Issue writes on this repo use the workflow's own `GITHUB_TOKEN`, not `RELEASE_TOKEN`.** `RELEASE_TOKEN` (a fine-grained PAT) is only for the account-wide read (`gh search prs`, `gh pr view` on other repos) that needs its broader reach; it can't assign issues at all - confirmed live via both the GraphQL path (`gh issue create --assignee`: `GraphQL: Resource not accessible by personal access token (replaceActorsForAssignable)`) and the REST path (`POST .../assignees`: flat `403 Resource not accessible by personal access token`), so this isn't a GraphQL-specific bug to route around, it's the token. `GITHUB_TOKEN` has no such restriction for issues in its own repo, and the workflow already runs in that repo - so `scripts/watchdog.sh` takes a second token, `GH_REPO_TOKEN`, and every `gh issue`/assignee call goes through a `gh_repo()` wrapper that swaps `GH_TOKEN` to it for just that call. A failed assignment still logs a `::warning::` and moves on rather than aborting the run, as defense in depth, not as the primary fix.
