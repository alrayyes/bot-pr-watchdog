...
- **Assign via the REST `POST /issues/{n}/assignees` endpoint, not `gh issue create --assignee`/`gh issue edit --add-assignee`.** Both of those go through a GraphQL mutation (`replaceActorsForAssignable`/`addAssigneesToAssignable`) that `RELEASE_TOKEN`, a fine-grained PAT, can't use - confirmed live on the very first real run: `GraphQL: Resource not accessible by personal access token (replaceActorsForAssignable)`, which killed the whole script under `set -e` before this fix. The REST endpoint is a different code path and isn't affected. A failed assignment now logs a `::warning::` and moves on rather than aborting the run - assignment not landing on one issue shouldn't stop the rest of that poll's opens/closes.
